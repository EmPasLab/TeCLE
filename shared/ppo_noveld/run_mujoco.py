...
"""
PPO-NovelD agent on MuJoCo continuous control environments.

From the paper "NovelD: A Simple yet Effective Exploration Criterion"
https://arxiv.org/abs/2110.09537
"""
...
flags.DEFINE_string('results_csv_path', './logs/ppo_noveld_mujoco_results.csv', 'Path for CSV log file.')
flags.DEFINE_string('checkpoint_dir', './checkpoints', 'Path for checkpoint directory.')
flags.DEFINE_string('load_checkpoint', '', 'Path to checkpoint file to restore from, default empty (no restore).')


def main(argv):
    """Trains PPO-NovelD agent on MuJoCo continuous control tasks."""
    del argv
    runtime_device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    logging.info(f'Runs PPO-NovelD-Gaussian agent on {runtime_device}')
    ...
    # Create PPO-NovelD Gaussian learner agent instance.
    ...
    eval_agent = greedy_actors.GaussianRndPolicyGreedyActor(
        ...
        device=runtime_device,
        name='PPO-NovelD-Gaussian-greedy',
    )

    # Setup checkpoint.
    checkpoint = PyTorchCheckpoint(
        environment_name=FLAGS.environment_name, agent_name='PPO-NovelD-Gaussian', save_dir=FLAGS.checkpoint_dir
    )
