...
"""
PPO-ICM agent on MuJoCo continuous control environments.
...
from networks.curiosity import GaussianIcmMlpNet
from checkpoint import PyTorchCheckpoint
from schedule import LinearSchedule
import main_loop
import gym_env
from ppo_icm import agent
...
flags.DEFINE_float('icm_learning_rate', 0.0003, 'Learning rate for ICM module.')
...
flags.DEFINE_float('policy_loss_coef', 1.0, 'Weights policy loss against the ICM module loss.')
...
flags.DEFINE_string('results_csv_path', './logs/ppo_icm_mujoco_results.csv', 'Path for CSV log file.')
flags.DEFINE_string('checkpoint_dir', './checkpoints', 'Path for checkpoint directory.')
flags.DEFINE_string('load_checkpoint', '', 'Path to checkpoint file to restore from, default empty (no restore).')


def main(argv):
    """Trains PPO-ICM agent on MuJoCo continuous control tasks."""
    del argv
    ...
    # ICM module
    icm_network = GaussianIcmMlpNet(state_dim=state_dim, action_dim=action_dim)
