...
class FileWriter:
    def __init__(self,
        ...
        if not xpid:
            if noisy == True:
                # make unique id
                xpid = '{proc}_{unixtime}_noisy_tv_true'.format(proc=os.getpid(), unixtime=int(time.time()))
            else:
                xpid = '{proc}_{unixtime}_noisy_tv_false'.format(proc=os.getpid(), unixtime=int(time.time()))
        self.xpid = xpid
        self._tick = 0
