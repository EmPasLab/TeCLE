...
from networks.policy import ActorCriticConvNet
from networks.curiosity import LpmConvNet
...
flags.DEFINE_float('learning_rate', 0.00045, 'Learning rate for policy network.')
flags.DEFINE_float('lpm_learning_rate', 0.001, 'Learning rate for LPM network (dynamics + error heads).')
...
flags.DEFINE_float('int_coeff', 1.0, 'Coefficient for the intrinsic (LPM) reward.')
...
flags.DEFINE_string('results_csv_path', './logs/ppo_noisy_atari_results.csv', 'Path for CSV log file.')
flags.DEFINE_string('checkpoint_dir', './checkpoints', 'Path for checkpoint directory.')


def main(argv):
    """Trains PPO-Noisy agent on Atari."""
    del argv
    runtime_device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    logging.info(f'Runs PPO-Noisy agent on {runtime_device}')
    ...
    # Create LPM network.
    lpm_network = LpmConvNet(state_dim=state_dim, action_dim=action_dim)
    ...
    eval_agent = greedy_actors.PolicyGreedyActor(
        ...
        device=runtime_device,
        name='PPO-Noisy-greedy',
    )

    checkpoint = PyTorchCheckpoint(
        environment_name=FLAGS.environment_name, agent_name='PPO-Noisy', save_dir=FLAGS.checkpoint_dir
    )
