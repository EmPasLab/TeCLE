...
MinigridCVAE = models.MinigridCVAE
...
def learn(actor_model,
    ...
    with lock:
        ...
        recon_next_state_emb, mean_t, logvar_t, _ = cvae_network_model(next_state_emb, batch['action'][1:])
        pred_actions = inverse_dynamics_model(state_emb, next_state_emb)
        intrinsic_rewards = torch.norm(recon_next_state_emb.detach() - next_state_emb.detach(), dim=2, p=2)
        ...
        cvae_network_loss = cvae_loss(recon_next_state_emb.detach(), next_state_emb, mean_t, logvar_t)
        ...
        total_loss = pg_loss + baseline_loss + entropy_loss + inverse_dynamics_loss + cvae_network_loss
        ...
        stats = {
            ...
            'cvae_network_loss': cvae_network_loss.item(),
        ...
        cvae_network_optimizer.zero_grad()
...
def train(flags):  
    ...
    if 'MiniGrid' in flags.env: 
        ...
        if flags.use_fullobs_intrinsic:                        
            ...
            cvae_network = MinigridCVAE(env.observation_space.shape).to(device=flags.device)   
    ...
    cvae_network_optimizer = torch.optim.RMSprop(
    ...
    def batch_and_learn(i, lock=threading.Lock()):
        ...
        while frames < flags.total_frames:
            ...
            stats = learn(model, learner_model, random_target_network, predictor_network,
                          batch, agent_state, optimizer, state_embedding_optimizer, 
                          cvae_network_optimizer, inverse_dynamics_optimizer, scheduler, 
                          flags, frames=frames)
            timings.time('learn')
            with lock:
    ...
    def checkpoint(frames):
        ...
        torch.save({
            ...
            'cvae_network_optimizer_state_dict': cvae_network_optimizer.state_dict(),
