...
def build_command(
    ...
    script = SHARED_DIR / algo / 'run_mujoco.py'
    ...
    if noise_beta is not None and algo == 'ppo_tecle':
        cmd.append(f'--noise_beta={noise_beta}')
...
def run_experiment(
    ...
    cmd = build_command(algo, env, seed, num_iterations, num_actors, noise_beta, load_checkpoint)
...
def main():
    parser = argparse.ArgumentParser(description='Run MuJoCo / Gym-Robotics benchmarks.')
    parser.add_argument(
        '--algorithms',
        nargs='+',
        choices=['ppo', 'ppo_icm', 'ppo_rnd', 'ppo_noveld', 'ppo_disagree', 'ppo_ama', 'ppo_tecle', 'ppo_noisy'],
        default=ALGORITHMS,
