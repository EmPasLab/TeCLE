...
def create_env(flags):
    ...
    if 'MiniGrid' in flags.env:
        if flags.Noisy_TV is True:
            print("Noisy TV setting is True")
            return Minigrid2Image(wrappers.FullyObsWrapper(atari_wrappers.NoisyTVWrapper(gym.make(flags.env), True)))
        else:
            print("Noisy TV setting is False")
...
def create_buffers(obs_shape, num_actions, flags) -> Buffers:
    ...
    specs = dict(
        ...
        done=dict(size=(T + 1,), dtype=torch.bool), # origin is uint8
