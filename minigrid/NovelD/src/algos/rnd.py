...
def train(flags):  
    ...
    def batch_and_learn(i, lock=threading.Lock()):
        ...
        while frames < flags.total_frames:
            ...
            stats = learn(model, learner_model, random_target_network, predictor_network,
                          encoder, learner_encoder, batch, agent_state, encoder_state, optimizer, predictor_optimizer, scheduler, 
                          flags, frames=frames)
            timings.time('learn')
            with lock:
    ...
    def save_heatmap(frames):
        checkpoint_path = os.path.expandvars(os.path.expanduser(
            '%s/%s/%s' % (flags.savedir, flags.xpid,'heatmap_'+str(frames)+'.json')))
        log.info('Saving heatmap to %s', checkpoint_path)
        heatmap_dict = dict()
        for i, key in enumerate(heatmap_buffers.keys()):
            heatmap_dict.update({i: heatmap_buffers[key].item()})
        with open(checkpoint_path, 'w') as fp:
            json.dump(heatmap_dict, fp)
