...
"""
PPO-AMA agent on Atari environments.

AMA (Aleatoric Modelling for Agents) uses a probabilistic forward model that
predicts both mean and log-variance of next-state features. The intrinsic
reward is the prediction error minus the predicted aleatoric uncertainty:
    reward = max(0, mse - exp(log_var))

This avoids rewarding stochastic but uncontrollable observations (noisy TVs).
...
from networks.curiosity import AmaNatureConvNet
...
flags.DEFINE_float('ama_learning_rate', 0.001, 'Learning rate for AMA module.')
...
flags.DEFINE_bool('actors_on_gpu', False, 'Run actors on GPU, default off.')
flags.DEFINE_integer(
...
flags.DEFINE_string('results_csv_path', './logs/ppo_ama_BankHeist_results.csv', 'Path for CSV log file.')
flags.DEFINE_string('checkpoint_dir', './checkpoints', 'Path for checkpoint directory.')


def main(argv):
    """Trains PPO-AMA agent on Atari."""
    del argv
    ...
    # Create AMA module (aleatoric forward model + inverse model).
    ama_network = AmaNatureConvNet(state_dim=state_dim, action_dim=action_dim)
    ...
    # Store copy of latest parameters in a shared dictionary for actors.
    shared_params = manager.dict({'policy_network': None})

    # Create PPO-AMA learner agent instance.
