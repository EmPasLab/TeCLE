...
"""
PPO agent on MuJoCo continuous control environments.
...
flags.DEFINE_string('results_csv_path', './logs/ppo_mujoco_results.csv', 'Path for CSV log file.')
flags.DEFINE_string('checkpoint_dir', './checkpoints', 'Path for checkpoint directory.')
flags.DEFINE_string('load_checkpoint', '', 'Path to checkpoint file to restore from, default empty (no restore).')


def main(argv):
    """Trains PPO agent on MuJoCo continuous control tasks."""
    del argv
    ...
    checkpoint = PyTorchCheckpoint(
        environment_name=FLAGS.environment_name, agent_name='PPO', save_dir=FLAGS.checkpoint_dir
    )
