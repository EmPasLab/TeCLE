...
from networks.curiosity import RndConvNet
...
flags.DEFINE_string('results_csv_path', './logs/ppo_noveld_BankHeist_results.csv', 'Path for CSV log file.')
flags.DEFINE_string('checkpoint_dir', './checkpoints', 'Path for checkpoint directory.')


def main(argv):
    """Trains PPO-NovelD agent on Atari."""
    del argv
    runtime_device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    logging.info(f'Runs PPO-NovelD agent on {runtime_device}')
    ...
    eval_agent = greedy_actors.PolicyGreedyActor(
        ...
        device=runtime_device,
        name='PPO-NovelD-greedy',
    )

    # Setup checkpoint.
    checkpoint = PyTorchCheckpoint(
        environment_name=FLAGS.environment_name, agent_name='PPO-NovelD', save_dir=FLAGS.checkpoint_dir
    )
