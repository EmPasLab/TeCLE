...
class Actor(types_lib.Agent):
    ...
    def _choose_action(self, timestep: types_lib.TimeStep) -> Tuple[types_lib.Action]:
        ...
        s_t = torch.from_numpy(timestep.observation[None, ...]).to(device=self._device, dtype=torch.float32) # 새로운 차원 생성함을 의미
...
class Learner(types_lib.Learner):
    ...
    def statistics(self) -> Mapping[Text, float]:
        ...
        return {
            ...
            'intrinsic reward': self.intrinsic_reward_t,
        }
...
class GaussianActor(Actor):
    """PPO-ICM Gaussian actor for continuous action space."""
    ...
    def __init__(
        ...
        self.agent_name = f'PPO-ICM-Gaussian-actor{rank}'
...
class GaussianLearner(types_lib.Learner):
    """PPO-ICM Gaussian learner for continuous action space."""
    ...
    def __init__(
        ...
        if not 0.0 <= icm_beta <= 1.0:
            raise ValueError(f'Expect icm_beta in [0.0, 1.0], got {icm_beta}')
        if not 0.0 <= policy_loss_coef <= 1.0:
        ...
        self.agent_name = 'PPO-ICM-GaussianLearner'
        self._policy_network = policy_network.to(device=device)
    ...
    def _update_icm_network(self, transitions: GaussianTransition) -> IcmModuleOutput:
        self._icm_optimizer.zero_grad()
        loss, icm_output = self._calc_icm_loss(transitions=transitions)
        loss.backward()
    ...
    def _update_policy_network(self, transitions: GaussianTransition, icm_output: IcmModuleOutput) -> None:
        self._policy_optimizer.zero_grad()
        loss = self._calc_policy_loss(transitions, icm_output)
        loss.backward()
    ...
    def _calc_icm_loss(self, transitions: GaussianTransition) -> Tuple[torch.Tensor, IcmModuleOutput]:
        ...
        icm_output = self._icm_network(s_t, a_t, s_tp1)
        pred_action = icm_output.pred_action
        pred_features_t = icm_output.pred_features
        features_t = icm_output.features

        inverse_losses = F.mse_loss(pred_action, a_t, reduction='none').mean(dim=-1)
        forward_losses = torch.mean(0.5 * torch.square(pred_features_t - features_t.detach()), dim=1)

        if len(forward_losses.shape) != 1:
            raise RuntimeError(f'Expect forward_losses to be a vector, got {forward_losses.shape}')
    ...
    def _calc_policy_loss(self, transitions: GaussianTransition, icm_output: IcmModuleOutput) -> torch.Tensor:
        s_t = torch.from_numpy(transitions.s_t).to(device=self._device, dtype=torch.float32)
        a_t = torch.from_numpy(transitions.a_t).to(device=self._device, dtype=torch.float32)
