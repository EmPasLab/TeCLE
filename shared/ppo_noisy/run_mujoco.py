...
"""
PPO-Noisy agent on MuJoCo continuous control environments.

Noise-robust exploration via Learning Progress Monitoring (LPM): intrinsic
reward = g_phi(s_t, a_t) - log_MSE(f_theta(s_t, a_t), phi(s_{t+1})).
Rewards genuine learning progress; gives ~0 reward for unlearnable noise.
...
from networks.curiosity import LpmMlpNet
from ppo_noisy import agent
...
flags.DEFINE_float('lpm_learning_rate', 0.0003, 'Learning rate for LPM network (dynamics + error heads).')
...
flags.DEFINE_float('int_coeff', 0.01, 'Coefficient for the intrinsic (LPM) reward.')
flags.DEFINE_integer('error_buffer_size', 4096, 'Max number of (s_t, a_t, log_mse) entries in error buffer.')
...
flags.DEFINE_string('results_csv_path', './logs/ppo_noisy_mujoco_results.csv', 'Path for CSV log file.')
flags.DEFINE_string('checkpoint_dir', './checkpoints', 'Path for checkpoint directory.')
flags.DEFINE_string('load_checkpoint', '', 'Path to checkpoint file to restore from, default empty (no restore).')


def main(argv):
    """Trains PPO-Noisy agent on MuJoCo continuous control tasks."""
    del argv
    runtime_device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    logging.info(f'Runs PPO-Noisy-Gaussian agent on {runtime_device}')
    ...
    # Create LPM network.
    lpm_network = LpmMlpNet(state_dim=state_dim, action_dim=action_dim)
    lpm_optimizer = torch.optim.Adam(lpm_network.parameters(), lr=FLAGS.lpm_learning_rate)
    ...
    learner_agent = agent.GaussianLearner(
        ...
        lpm_network=lpm_network,
        lpm_optimizer=lpm_optimizer,
        ...
        error_buffer_size=FLAGS.error_buffer_size,
    ...
    eval_agent = greedy_actors.GaussianPolicyGreedyActor(
        ...
        device=runtime_device,
        name='PPO-Noisy-Gaussian-greedy',
    )

    checkpoint = PyTorchCheckpoint(
        environment_name=FLAGS.environment_name, agent_name='PPO-Noisy-Gaussian', save_dir=FLAGS.checkpoint_dir
    )
    checkpoint.register_pair(('policy_network', policy_network))
    checkpoint.register_pair(('critic_network', critic_network))
    checkpoint.register_pair(('lpm_network', lpm_network))
