...
"""PPO-NovelD agent class.

From the paper "NovelD: A Simple yet Effective Exploration Criterion"
...
class Actor(types_lib.Agent):
    ...
    def __init__(
        ...
        self.rank = rank
        self.agent_name = f'PPO-NovelD-actor{rank}'
...
class Learner(types_lib.Learner):
    ...
    def __init__(
        ...
        self.agent_name = 'PPO-NovelD-learner'
        self._policy_network = policy_network.to(device=device)
    ...
    def received_item_from_queue(self, unroll_sequences: Iterable[Tuple]) -> None:
        ...
        # Get observation for NovelD: need last frame of s_t and s_{t+1}
        rnd_s_t = [s[-1:, ...] for s in s_t]
        s_tp1 = observations[1:]
        rnd_s_tp1 = [s[-1:, ...] for s in s_tp1]

        # Compute intrinsic rewards: max(0, alpha * e(s_{t+1}) - e(s_t))
        int_r_t = self._compute_int_reward(rnd_s_t, rnd_s_tp1)
    ...
    @torch.no_grad()
    def _compute_int_reward(self, rnd_s_t, rnd_s_tp1, alpha=1.0):
        """NovelD intrinsic reward: max(0, alpha * e(s_{t+1}) - e(s_t))
        where e(s) = ||pred(s) - target(s)||^2
        """
        normed_s_t = self._normalize_rnd_obs(rnd_s_t).to(device=self._device, dtype=torch.float32)
        normed_s_tp1 = self._normalize_rnd_obs(rnd_s_tp1).to(device=self._device, dtype=torch.float32)

        pred_t = self._rnd_predictor_network(normed_s_t)
        target_t = self._rnd_target_network(normed_s_t)
        e_t = torch.square(pred_t - target_t).mean(dim=1)

        pred_tp1 = self._rnd_predictor_network(normed_s_tp1)
        target_tp1 = self._rnd_target_network(normed_s_tp1)
        e_tp1 = torch.square(pred_tp1 - target_tp1).mean(dim=1)

        int_r_t = torch.clamp(alpha * e_tp1 - e_t, min=0.0).detach().cpu().numpy()
...
class GaussianActor(Actor):
    """PPO-NovelD actor for continuous action spaces using a Gaussian policy."""

    @torch.no_grad()
    def _choose_action(self, timestep: types_lib.TimeStep) -> Tuple[types_lib.Action, float, float, float]:
        """Given timestep, choose continuous action a_t from Gaussian distribution."""
        s_t = torch.from_numpy(timestep.observation[None, ...]).to(device=self._device, dtype=torch.float32)
        output = self._policy_network(s_t)
        pi_mu = output.pi_mu
        pi_sigma = output.pi_sigma
        ...
        return (
            ...
            logprob_a_t.squeeze(0).cpu().numpy(),
            ext_value.squeeze(0).cpu().item(),
            int_value.squeeze(0).cpu().item(),
        )
...
class GaussianLearner(types_lib.Learner):
    """PPO-NovelD learner for continuous action spaces (MuJoCo) using a Gaussian policy."""
    ...
    def __init__(
        ...
        if not 0.0 <= rnd_experience_proportion <= 1.0:
            raise ValueError(f'Expect rnd_experience_proportion in [0, 1], got {rnd_experience_proportion}')
        if not 0.0 <= entropy_coef <= 1.0:
            raise ValueError(f'Expect entropy_coef in [0, 1], got {entropy_coef}')
        if not 0.0 <= value_coef <= 1.0:
            raise ValueError(f'Expect value_coef in [0, 1], got {value_coef}')

        self.agent_name = 'PPO-NovelD-Gaussian-learner'
        self._policy_network = policy_network.to(device=device)
        self._policy_optimizer = policy_optimizer
        self._rnd_predictor_network = rnd_predictor_network.to(device=device)
        self._rnd_target_network = rnd_target_network.to(device=device)
        ...
        self._int_reward_normalizer = normalizer.RunningMeanStd(shape=(1,))
        # For MuJoCo: normalize 1-D state vectors
        self._rnd_obs_normalizer = normalizer.TorchRunningMeanStd(shape=(state_dim,), device=self._device)
    ...
    def received_item_from_queue(self, unroll_sequences: Iterable[Tuple]) -> None:
        ...
        # For MuJoCo: use the full state vector
        rnd_s_t = s_t
        rnd_s_tp1 = observations[1:]
        int_r_t = self._compute_int_reward(rnd_s_t, rnd_s_tp1)
    ...
    def _update_policy_network(self, mini_batch):
        ...
        pi_mu = output.pi_mu
        pi_sigma = output.pi_sigma
        ext_v_t = output.ext_baseline
        int_v_t = output.int_baseline

        pi_dist_t = distributions.normal_distribution(pi_mu, pi_sigma)
        pi_logprob_a_t = pi_dist_t.log_prob(a_t).sum(axis=-1)
        entropy_loss = pi_dist_t.entropy().sum(axis=-1)
    ...
    def _compute_int_reward(self, rnd_s_t, rnd_s_tp1, alpha=1.0):
        ...
        int_r_t = torch.clamp(alpha * e_tp1 - e_t, min=0.0).detach().cpu().numpy()
        return self._normalize_int_rewards(int_r_t)

    @torch.no_grad()
    def _normalize_rnd_obs(self, rnd_obs_list, update_stats=False):
        stacked_obs = torch.from_numpy(np.stack(rnd_obs_list, axis=0)).to(device=self._device, dtype=torch.float32)
        normed_obs = self._rnd_obs_normalizer.normalize(stacked_obs)
        normed_obs = normed_obs.clamp(-self._rnd_obs_clip, self._rnd_obs_clip)
        if update_stats:
            self._rnd_obs_normalizer.update(stacked_obs)
        return normed_obs
