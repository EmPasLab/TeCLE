...
from networks.curiosity import TecleCVAEMlpNet as GaussianCVAE, TecleInverseMlpNet as GaussianInverseNet
...
flags.DEFINE_string(
    'environment_name',
    'HalfCheetah-v4',
    'MuJoCo continuous control environment name, e.g. HalfCheetah-v4, Hopper-v4, Ant-v4, Humanoid-v4.',
)
...
flags.DEFINE_float('tecle_learning_rate', 0.0003, 'Learning rate for TeCLE cVAE network.')
flags.DEFINE_float('inv_learning_rate', 0.0003, 'Learning rate for inverse dynamics network.')
flags.DEFINE_float('ext_discount', 0.99, 'Discount rate for extrinsic environment reward.')
flags.DEFINE_float('int_discount', 0.5, 'Discount rate for intrinsic reward.')
...
flags.DEFINE_integer(
    'init_obs_steps',
    128,
    'Number of random steps to warm-start the observation normalizer.',
)
flags.DEFINE_integer('obs_clip', 5, 'Observation normalization clip range.')
...
flags.DEFINE_integer('unroll_length', 2048, 'Collect N transitions before sending to learner, per actor.')
...
flags.DEFINE_float('noise_beta', 2.0, 'Colored noise beta for GaussianCVAE sampling (0=white, 1=pink, 2=red/brown).')
flags.DEFINE_string('tag', '', 'Add tag to Tensorboard log file.')
flags.DEFINE_string('results_csv_path', './logs/ppo_tecle_mujoco_results.csv', 'Path for CSV log file.')
...
def main(argv):
    ...
    logging.info(f'Runs PPO-TeCLE-Gaussian agent on {runtime_device}')
    ...
    feature_dim = FLAGS.hidden_size
    inv_network = GaussianInverseNet(state_dim=state_dim, action_dim=action_dim, hidden_size=FLAGS.hidden_size)
    tecle_network = GaussianCVAE(feat_dim=feature_dim, action_dim=action_dim, noise_beta=FLAGS.noise_beta)

    policy_optimizer = torch.optim.Adam(policy_network.parameters(), lr=FLAGS.learning_rate)
    tecle_optimizer = torch.optim.Adam(tecle_network.parameters(), lr=FLAGS.tecle_learning_rate)
    inv_optimizer = torch.optim.Adam(inv_network.parameters(), lr=FLAGS.inv_learning_rate)
    ...
    eval_agent = greedy_actors.GaussianRndPolicyGreedyActor(
        ...
        device=runtime_device,
        name='PPO-TeCLE-Gaussian-greedy',
    )

    checkpoint = PyTorchCheckpoint(
        environment_name=FLAGS.environment_name, agent_name='PPO-TeCLE-Gaussian', save_dir=FLAGS.checkpoint_dir
    )
