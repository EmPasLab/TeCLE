...
"""
PPO-Disagree agent on MuJoCo continuous control environments.

Exploration by ensemble disagreement: intrinsic reward is the variance of
next-state feature predictions across an ensemble of forward dynamics models.
...
from networks.curiosity import DisagreeMlpNet
from ppo_disagree import agent
...
flags.DEFINE_float('baseline_learning_rate', 0.0003, 'Learning rate for critic network.')
flags.DEFINE_float('disagree_learning_rate', 0.0003, 'Learning rate for disagree ensemble network.')
...
flags.DEFINE_integer('num_ensemble', 5, 'Number of forward models in the disagreement ensemble.')
flags.DEFINE_float('ext_coeff', 1.0, 'Coefficient for the extrinsic reward.')
flags.DEFINE_float('int_coeff', 0.01, 'Coefficient for the intrinsic (disagreement) reward.')
...
flags.DEFINE_string('results_csv_path', './logs/ppo_disagree_mujoco_results.csv', 'Path for CSV log file.')
flags.DEFINE_string('checkpoint_dir', './checkpoints', 'Path for checkpoint directory.')
flags.DEFINE_string('load_checkpoint', '', 'Path to checkpoint file to restore from, default empty (no restore).')


def main(argv):
    """Trains PPO-Disagree agent on MuJoCo continuous control tasks."""
    del argv
    runtime_device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    logging.info(f'Runs PPO-Disagree-Gaussian agent on {runtime_device}')
    ...
    # Create disagreement ensemble network.
    disagree_network = DisagreeMlpNet(state_dim=state_dim, action_dim=action_dim, num_ensemble=FLAGS.num_ensemble)
    disagree_optimizer = torch.optim.Adam(disagree_network.parameters(), lr=FLAGS.disagree_learning_rate)
    ...
    # Create PPO-Disagree Gaussian learner agent instance.
    learner_agent = agent.GaussianLearner(
        policy_network=policy_network,
        policy_optimizer=policy_optimizer,
        critic_network=critic_network,
        critic_optimizer=critic_optimizer,
        disagree_network=disagree_network,
        disagree_optimizer=disagree_optimizer,
        ...
        ext_coeff=FLAGS.ext_coeff,
        int_coeff=FLAGS.int_coeff,
    ...
    eval_agent = greedy_actors.GaussianPolicyGreedyActor(
        ...
        device=runtime_device,
        name='PPO-Disagree-Gaussian-greedy',
    )

    # Setup checkpoint.
    checkpoint = PyTorchCheckpoint(
        environment_name=FLAGS.environment_name, agent_name='PPO-Disagree-Gaussian', save_dir=FLAGS.checkpoint_dir
    )
    checkpoint.register_pair(('policy_network', policy_network))
    checkpoint.register_pair(('critic_network', critic_network))
    checkpoint.register_pair(('disagree_network', disagree_network))
